...
public class BaseService<D extends BaseDTO, T> implements IBaseService<D, T> {
  ...
  public BaseService(BaseRepository<T, Integer> repository, Class<D> dtoClass, Class<T> entityClass) {
  ...
  public D atualizar(D dto) {
    T saved = repository.findById(dto.getId()).orElseThrow(this::notFound);
  ...
  public List<D> listarComFiltro(List<Filtro> filtros, Class<? extends BaseDTO> aClass) {
